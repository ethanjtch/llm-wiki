...
## [2026-09-16] ingest | Steph Ango 知识管理四件套（处理 lint 发现）

处理四次 lint 发现的待摄入 raw 文件（4 篇均为 Steph Ango 短文）。来源页 4：[[常青笔记将想法转化为你可以操控的对象（Steph Ango）]]（2022，evergreen notes 对象化；术语创自 Andy Matuschak）、[[不要把理解力拱手让人（Steph Ango）]]（2023，寄生虫三阶段：接受→榨取→干预）、[[我如何完成待办事项（Steph Ango）]]（2023，周清单+顺延/丢弃+从零重写）、[[我如何使用 Obsidian（Steph Ango）]]（vault 模板：少文件夹多链接、分形日记、随机重访、file over app）。概念页 4：[[常青笔记]]、[[认知外包]]（执行可外包、判断不可——与[[被推迟工作的经济学]]的人工评审结论互证）、[[文件优先]]、[[涌现式笔记法]]（红链=面包屑，与 wiki 待建页同构）。更新 [[Steph Ango]]（实体，sources 1→5）、[[LLM Wiki 模式]]（补思想谱系 Memex→常青笔记→LLM wiki，并记录本文库最大的正面争论：Ango 拒绝把维护交给 LLM vs Karpathy 全部外包，裁决=记账可外包、理解不外包）、[[个人工单系统]]（Ango 周清单净化机制对照）、[[工具敬畏]]（姐妹篇：管手 vs 管脑）。更新 index（6 实体/27 概念/13 来源/7 综合）与首页。

## [2026-09-16] archive | 归档失效来源页 Gemini in Chrome 开启教程（CSDN）

按四次 lint 的裁决执行：raw 原文件已被删除，且该文属时效性配置 Hack（违背入库门槛），经 Obsidian CLI 移入回收站。同时移除 [[平台与生态之争]]、[[封闭生态]] 中的强链接（论点保留为一般性机制描述，不再指向已归档页）、index 条目与体检报告"待裁决"段（改为"已处理"）。log 与 index 计数同步更新。
